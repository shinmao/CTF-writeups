 <?php
  ...
  if(isset($_GET["username"]) && isset($_GET["password"])){
    ...
    for($i=0;$i<16;$i++) 
        $random[] = strtr(base64_encode(openssl_random_pseudo_bytes(48)),"+/","-_");

    $csvdb = "";

    for($i=0;$i<8;$i++) 
        $csvdb .= $random[$i*2].",".$random[$i*2+1]."\n";

    $sqldb = new SQLite3(":memory:");
    // users 兩欄位 username, password
    $sqldb->exec("CREATE TABLE users (username text, password text)");

    for($i=0;$i<8;$i++) 
        $sqldb->exec("INSERT INTO users VALUES ('".$random[$i*2]."', '".$random[$i*2+1]."')");
